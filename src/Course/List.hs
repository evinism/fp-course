...
product ::
  ...
  -> Int
product Nil = 1
product (top :. rest) = top * product rest
...
map ::
  ...
  -> List b
map _ Nil = Nil
map predicate (first :. rest) =  predicate first :. map predicate rest
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap predicate list = flatten (map predicate list)
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional (Empty :. rest) = Empty
seqOptional (Nil) = Full Nil
seqOptional (Full a :. rest) = mapOptional (\val -> a :. val) $ seqOptional rest
...
find ::
  ...
  -> List a
  -> Optional a
find predicate Nil = Empty
find predicate (first :. rest) = if predicate first then Full first else find predicate rest
